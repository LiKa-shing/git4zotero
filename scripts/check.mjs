...
assert(mainModule.includes("RepositoryArchiveService"));
assert(mainModule.includes("archiveService: this.archive"));
...
assert(platformModule.includes("tryPickOpenFileWithNativeDialog"));
assert(platformModule.includes("createWindowsOpenFilePickerScripts"));
assert(platformModule.includes("createWindowsFormsOpenFilePickerScript"));
assert(platformModule.includes("createWindowsWpfOpenFilePickerScript"));
assert(platformModule.includes("OpenFileDialog"));
assert(platformModule.includes("promptOpenFilePath"));
assert(platformModule.includes("assertFileAvailable"));
assert(platformModule.includes("openFileDialogUnavailable"));
assert(platformModule.includes("archiveImportPathPromptMessage"));
...
assert(archiveModule.includes("readImportArchive"));
assert(archiveModule.includes("validateArchiveManifest"));
assert(archiveModule.includes("groupImportRepositoryEntries"));
assert(archiveModule.includes("importRepositoryGroup"));
...
assert(styleModule.includes("touch-action: manipulation"));
assert(!styleModule.includes("git4zotero-version-detail-dialog"));
assert(!styleModule.includes("git4zotero-version-detail-backdrop"));
assert(!styleModule.includes("position: fixed"));
assert(!styleModule.includes("inset: 0"));
assert(!styleModule.includes("2147483647"));
...
assert(preferencesXhtml.includes("条目级导入/导出"));
assert(!preferencesXhtml.includes("git4zotero-export-history"));
assert(!preferencesXhtml.includes("git4zotero-import-history"));
assert(!preferencesXhtml.includes("onclick=\"Git4ZoteroPreferences.exportHistoryArchive(event)\""));
assert(!preferencesXhtml.includes("onclick=\"Git4ZoteroPreferences.importHistoryArchive(event)\""));
...
const runtimeConstantsScript = await fs.readFile(path.join(root, "chrome/content/src/constants.mjs"), "utf8");
const runtimeLocalizationScript = await fs.readFile(path.join(root, "chrome/content/src/localization.mjs"), "utf8");
...
assert(!preferencesScript.includes("export-history-archive"));
...
assert(preferencesScript.includes("autoSaveArchiveExportDirectory"));
assert(preferencesScript.includes("savePendingArchiveExportDirectory"));
assert(preferencesScript.includes("addEventListener(\"change\""));
assert(preferencesScript.includes("addEventListener(\"blur\""));
assert(preferencesScript.includes("addEventListener(\"keydown\""));
assert(preferencesScript.includes("persistArchiveExportDirectoryValue"));
assert(preferencesScript.includes("confirmedArchiveExportDirectory"));
assert(preferencesScript.includes("getSavedArchiveExportDirectory"));
...
assert(!preferencesScript.includes("initialDirectory: this.getArchiveExportDirectoryInputValue()"));
assert(preferencesScript.includes("await this.savePendingArchiveExportDirectory()"));
...
assert(!runtimeConstantsScript.includes("archiveExportDirectorySave:"));
assert(!runtimeLocalizationScript.includes("archiveExportDirectorySave:"));
...
assert(uiModule.includes("omittedParagraphChangesDialog"));
assert(uiModule.includes("data-git4zotero-version-detail-close"));
assert(uiModule.includes("data-git4zotero-closing"));
assert(uiModule.includes("dismissVersionDetailPanel"));
assert(uiModule.includes("findVersionDetailPanels"));
assert(uiModule.includes("setVersionDetailButtonsExpanded"));
assert(uiModule.includes("aria-expanded"));
assert(uiModule.includes("addEventListener?.(\"pointerdown\""));
...
assert(!uiModule.includes("2147483647"));
